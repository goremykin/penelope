...
%s""" % (len(self), self.unique_headwords, str(self.has_synonyms), str(self.metadata))
    ...
    def group(
        ...
        if prefix_function is not None:
            ...
            try:
                spec = importlib.util.spec_from_file_location("prefix_module", prefix_function)
                prefix_module = importlib.util.module_from_spec(spec)
                spec.loader.exec_module(prefix_module)

                get_prefix = prefix_module.get_prefix
            except:
                pass
